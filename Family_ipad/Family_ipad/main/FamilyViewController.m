...
#define POST_BTNFRAME_A CGRectMake(32,662,412,73)
#define POST_BTNFRAME_B   CGRectMake(357,662,87,73)
#define POST_BTNFRAME_C   CGRectMake(23,19,30,30)
...
- (void)expandPostView:(id)sender
{
    [UIView animateWithDuration:0.3f
                     animations:^{
                         if (_postBtnView.frame.size.width == POST_BTNFRAME_B.size.width) {
                             _postBtnView.frame = POST_BTNFRAME_A;
                             [_postAddBtn setImage:[UIImage imageNamed:@"post_close.png"] forState:UIControlStateNormal];

                         }else{
                             _postBtnView.frame = POST_BTNFRAME_B;
                             [_postAddBtn setImage:[UIImage imageNamed:@"post_add.png"] forState:UIControlStateNormal];
...
- (void)showPostViewController:(UIButton *)sender{
    ...
    if (sender == _post_photoBtn)
        con.postView.currentAction =  @"发照片";
    else if(sender == _post_diaryBtn)
        con.postView.currentAction = @"发日记";
    else if(sender == _post_pmBtn)
        con.postView.currentAction = @"发私信";
    else if (sender == _post_actBtn)
        con.postView.currentAction = @"发活动";
    else if (sender == _post_hiBtn)
        con.postView.currentAction = @"我想说";

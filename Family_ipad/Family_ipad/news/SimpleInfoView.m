...
- (void)initInfoWithHeadUrlStr:(NSString*)_headUrlStr
    ...
    [headBtn setImageWithURL:[NSURL URLWithString:_headUrlStr] placeholderImage:[UIImage imageNamed:@"head_110.png"]];
...
- (void)initInfoWithHeadUrlStr:(NSString*)_headUrlStr
    ...
    [headBtn setImageForMyHeadButtonWithUrlStr:_headUrlStr plcaholderImageStr:@"head_110.png" size:MIDDLE];
    ...
    self.infoLbl.text = $str(@":  %@",_infoStr);
...
- (void)layoutSubviews {
    ...
    if (!self.operatorBtn) {//右边没有一个操作按钮的
        CGFloat maxWidth = _isFromTask ? (self.frame.size.width - 15 - nameLbl.frame.origin.x-40) : (self.frame.size.width - 15 - nameLbl.frame.origin.x - nameLbl.frame.size.width - 45);
        ...
        if (_isFamilyList) {//家人列表
            ...
            rightImgView.frame = (CGRect){.origin.x = nameLbl.frame.origin.x, .origin.y = infoLbl.frame.origin.y, .size = rightImgView.frame.size};
...
//            bgImgView.frame = (CGRect){.origin = bgImgView.frame.origin, .size.width = bgImgView.frame.size.width, .size.height = self.frame.size.height};
            ...
            rightImgView.frame = (CGRect){.origin.x = nameLbl.frame.origin.x, .origin.y = infoLbl.frame.origin.y + infoLbl.frame.size.height+5, .size = rightImgView.frame.size};
        }
    }
}

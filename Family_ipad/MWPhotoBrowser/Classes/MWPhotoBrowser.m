...
- (void)viewDidLoad {
	...
	// View
	self.view.backgroundColor = mwbgColor();
	...
	_pagingScrollView.backgroundColor = mwbgColor();
    ...
    // Toolbar Items
    _previousButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"back2.png"] style:UIBarButtonItemStylePlain target:self action:@selector(gotoPreviousPage)];
    _nextButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"re_big2.png"] style:UIBarButtonItemStylePlain target:self action:@selector(gotoNextPage)];
        //By Aevit
    _actionButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"down.png"] style:UIBarButtonItemStylePlain target:self action:@selector(actionButtonPressed:)];
    ...
    if ([[[UIDevice currentDevice] systemVersion] floatValue] > 4.9) {
        //iOS 5
        UIImage *toolBarBgImg = [UIImage imageNamed: @"light_menu_bg.png"];
        if ([_toolbar respondsToSelector:@selector(setBackgroundImage:forToolbarPosition:barMetrics:)]) {
            [_toolbar setBackgroundImage:toolBarBgImg forToolbarPosition:0 barMetrics:0];
        }
    } else {
        //iOS 4
        UIImageView *toolbarbg =[[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"light_menu_bg.png"]] autorelease];
        [_toolbar addSubview:toolbarbg];
    }
...
- (void)performLayout {
    ...
    fixedLeftSpace.width = 70; // To balance action button
...
//    if (_displayActionButton) [items addObject:_actionButton];
    ...
    if ([self.navigationController.viewControllers objectAtIndex:0] == self) {
        ...
        UIBarButtonItem *doneButton = [[[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"确定", nil) style:UIBarButtonItemStylePlain target:self action:@selector(doneButtonPressed:)] autorelease];
        // Set appearance
...
- (CGRect)frameForToolbarAtOrientation:(UIInterfaceOrientation)orientation {
	...
	return CGRectMake(285, self.view.bounds.size.height - height-43, 440, height);
}
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    [self showProgressHUDCompleteMessage: error ? NSLocalizedString(@"失败", @"Informing the user a process has failed") : NSLocalizedString(@"已保存", @"Informing the user an item has been saved")];
    [self hideControlsAfterDelay]; // Continue as normal...
}

...
- (void)connection:(NSURLConnection *)aConnection didReceiveResponse:(NSURLResponse *)response
{
    if (![response respondsToSelector:@selector(statusCode)] || [((NSHTTPURLResponse *)response) statusCode] < 400)
    {
        ...
        self.allImageSize = (float)response.expectedContentLength;//for progressview by Aevit
...
- (void)connection:(NSURLConnection *)aConnection didReceiveData:(NSData *)data
{
    ...
    if (self.progressive && expectedSize > 0 && [delegate respondsToSelector:@selector(imageDownloader:didUpdatePartialImage:)])
    {
        ...
        if ([delegate respondsToSelector:@selector(imageDownloader:didUpdatePartialImage:)]) {
            [delegate performSelector:@selector(imageDownloader:didUpdatePartialImage:) withObject:self];
            return;//for progressview by Aevit   下面是下载部分的，我不用到
        }
//        NSLog(@"%.2f / %.2f = %.5f", (double)totalSize, (double)expectedSize, ((double)totalSize / (double)expectedSize));

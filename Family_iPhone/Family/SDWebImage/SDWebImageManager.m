...
- (void)imageDownloader:(SDWebImageDownloader *)downloader didUpdatePartialImage:(UIImage *)image
{
    ...
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        if (aDownloader == downloader)
        {
            ...
            if (image && [delegate respondsToSelector:@selector(updateProgressView:)])//for progressview by Aevit
            {
				NSNumber *progress = [NSNumber numberWithDouble:((float)aDownloader.imageData.length / (float)aDownloader.allImageSize)];

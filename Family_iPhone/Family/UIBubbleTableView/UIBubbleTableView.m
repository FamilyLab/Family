...
- (void)initializator
{
    ...
    _refreshView = [[SSLoadingView alloc] initWithFrame:CGRectMake(0, -kHeaderHeight, self.bounds.size.width, kHeaderHeight)];
    ...
    [_refreshView.activityIndicatorView stopAnimating];
    _refreshView.activityIndicatorView.hidden = YES;
    _refreshView.textLabel.text = @"加载更多...";
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    if (scrollView.contentOffset.y < -kHeaderHeight) {
        //触发上拉刷新
        if ([self.bubbleDataSource respondsToSelector:@selector(addMoreDataOnTop:)]) {
            if (self.isLoaing == NO) {
                self.isLoaing = YES;
                [self.bubbleDataSource addMoreDataOnTop:self];
            }
        }
    }
}

- (void)setIsLoaing:(BOOL)isLoaing {
    [UIView beginAnimations:nil context:NULL];
	[UIView setAnimationDuration:.3];
    UIEdgeInsets currentInsets = self.contentInset;
    currentInsets.top = isLoaing ? 30 : 0;
    self.contentInset = currentInsets;
	[UIView commitAnimations];
    
    if (isLoaing) {
        [self.refreshView.activityIndicatorView startAnimating];
        self.refreshView.activityIndicatorView.hidden = NO;
    } else {
        [self.refreshView.activityIndicatorView stopAnimating];
        self.refreshView.activityIndicatorView.hidden = YES;
    }
}
...
//    return MAX(data.insets.top + data.view.frame.size.height + data.insets.bottom, self.showAvatars ? 52 : 0) + 20;
    return MAX(data.insets.top + data.view.frame.size.height + data.insets.bottom, self.showAvatars ? 35 : 0) + 15;
}

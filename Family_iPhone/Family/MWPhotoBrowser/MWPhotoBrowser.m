...
- (void)viewDidLoad {
    ...
    // Toolbar Items
    _previousButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"light_back.png"] style:UIBarButtonItemStylePlain target:self action:@selector(gotoPreviousPage)];
    if (!_isOnlyNeedBackBtn) {
        _nextButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"light_repost.png"] style:UIBarButtonItemStylePlain target:self action:@selector(gotoNextPage)];
        //By Aevit
        _actionButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"light_down.png"] style:UIBarButtonItemStylePlain target:self action:@selector(actionButtonPressed:)];
    }
//    _actionButton = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemAction target:self action:@selector(actionButtonPressed:)];
    ...
    if ([[[UIDevice currentDevice] systemVersion] floatValue] > 4.9) {
        ...
        //iOS 4
        [_toolbar insertSubview:[[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"light_menu_bg.png"]] autorelease] atIndex:0];
    }
...
- (void)savePhoto {
    id <MWPhoto> photo = [self photoAtIndex:_currentPageIndex];
    if ([photo underlyingImage]) {
        [self showProgressHUDWithMessage:@"保存中..."];
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    [self showProgressHUDCompleteMessage: error ? @"保存失败T_T" : @"保存成功"];

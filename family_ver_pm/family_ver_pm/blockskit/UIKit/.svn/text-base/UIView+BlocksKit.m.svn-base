...
//长按
- (void)whenLongPressedWithhandler:(BKBlock)block {
    if (!block) {
        return;
    }
    for (id obj in self.gestureRecognizers) {
        if ([obj isKindOfClass:[UILongPressGestureRecognizer class]]) {
            UILongPressGestureRecognizer *ges = (UILongPressGestureRecognizer*)obj;
            [self removeGestureRecognizer:ges];
        }
    }
    UILongPressGestureRecognizer *gesture = [UILongPressGestureRecognizer recognizerWithHandler:^(UIGestureRecognizer *sender, UIGestureRecognizerState state, CGPoint location) {
